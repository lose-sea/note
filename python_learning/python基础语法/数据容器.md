...
```python
list = ["hello", "world", "nihao", "xinyan"]

# 获取元素的索引
index = list.index("nihao")
print(index)

# 插入元素
list.insert(1, "messory")
print(list)
...
# 追加一批元素
# 列表.extend(数据容器), 将数据容器中的内容取出, 一次追加到列表尾部,
list1 = ["lala", "boy"]
list.extend(list1)
print(list) # ['hello', 'messory', 'world', 'nihao', 'xinyan', 'hehe', 'lala', 'boy']
...
功能：将字符串内的全部：字符串 1，替换为字符串 2

注意：不是修改字符串本身，而是得到了一个新字符串 

这里是将字符串中所有的 字符串1 子串都替换为 字符串2
```
...
2. `index() 查找某个元素在元组中索引位置 (第一次出现的位置), 否则报错`
3. `len(元组), 统计元组中的元素个数`
...
```python
# 消除两个集合的差集 
语法: s1.difference_updata(s2) 
功能: 对比集合一和集合二, 在集合一内,消除与集合二相同的yuan
```
...
普通字典访问**不存在的键**会直接报错 `KeyError`，而 `defaultdict` 会**给不存在的键自动生成一个默认值**

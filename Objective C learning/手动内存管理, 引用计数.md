...
一个对象创建出来的时候引用计数就是1 ,例入: `NSObject* obj = [[NSObject alloc] init];`此时对象的持有者就是 obj 
...
两个对象互相强持有, 导致谁的 retainCount都无法归零  

例子: 

````objc
Person *person = [[Person alloc] init]; // 你 持有 person
Dog *dog = [[Dog alloc] init];          // 你 持有 dog

person.dog = dog;    // person 持有 dog   → dog retainCount = 2
dog.owner = person;  // dog 持有 person   → person retainCount = 2

[person release];    // 你 放弃 person    → person retainCount = 1
[dog release];       // 你 放弃 dog       → dog retainCount = 1
```
...
剩下：
person ──retain──▶ dog    （dog retainCount = 1）
dog    ──retain──▶ person （person retainCount = 1）
````

在委托中委托方自委托, 代理方持有委托方, 委托方是自己,同时也是代理方, 就相当于委托方强引用代理方, 代理方强引用委托方, 造成循环引用
...
>
> **所有未被提交到git仓库的修改(包括工作区和暂存区)都会被彻底重置/覆盖, 且无法恢复** 
>
> `git reset --hard HEAD~1`   重置到上一个版本（最新提交的前一次） 
>
> `git reset --hard HEAD~2  `重置到上上个版本 
> `git reset --hard HEAD^`   和 HEAD~1 等价，也是上一个版本 
>
> `git reset`  只是把修改移除暂存区而不会撤销修改
>
> 在大多数情况下
>
> `git reset 等价于 git reset HEAD `
>
> - 只想 “取消 git add”：用 `git reset HEAD`（安全，不丢修改）；
> - 想 “撤销工作区的修改”：用 `git restore <文件>` 或 `git reset --hard`（后者会清空所有未提交修改）；
...
>
> **注意: `git rm --cached <文件>` 只会把文件移出暂存区，完全不会删除你对文件的任何修改**
